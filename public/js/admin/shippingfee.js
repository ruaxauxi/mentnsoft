$(document).ready(function(){
	...
	$(document).on('blur','#weight',function(){		
		var total = $.parseNumber($('#weight').val(),{format:"#,##0", locale:"us"});	
		total =  $.formatNumber(total,{format:"#,##0.00", locale:"us"});
		 $('#weight').val(total);
	...
	$(document).on('click','#btnSave',function(e){
		e.preventDefault();
			...
			var weight = $.parseNumber($('#weight').val(),{format:"#,##0", locale:"us"});	
			if (weight <= 0){
				$("#notify").find('span').text('Vui lòng nhập vào trọng lượng.');
				$("#notify").dialog({
					  my: "center",
					  at: "center",
					  of: window,
			...
			var param = {"btnSave":true,"shipment_id":shipment_id,'weight':weight,'total':total,"note":note,"date":date};
