$(document).ready(function(){
	...
	$(document).on('click','#btnSave',function(e){
		...
		var valid = true;
		$('.orderRow').find('.nick').each(function(i,e){
			if(!$(e).val()){
				valid = false;
		...
		if (!valid){
			e.preventDefault();
			$("#notify").find('span').text('Bạn chưa chọn nick.');
			$("#notify").dialog({
				 my: "center",
			     at: "center",
			     of: window,
	...
	$(document).on('click','#addRow',function(e){
		...
		$(newRow).append(cols);
		$(newRow).find('#hiddenNick').addClass('nick');
		$(newRow).find('#hiddenStore').addClass('store');

		$(newRow).find('#hiddenNick').removeAttr('id');
		$(newRow).find('#hiddenStore').removeAttr('id');
