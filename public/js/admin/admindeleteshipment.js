$(document).ready(function(){
	...
	$(document).on('click','.del',function(e){
		...
		$("#divDeleteForm").find('span').html('Có chắc bạn muốn xóa toàn bộ dữ liệu <span class="bigsize">' + $(this).data('shipname') + '</span>?');
			  ...
			  buttons: [
						      {
						    	...
						    	click: function(){
							    		...
							    		var shipment_id = $(tag).data('shipmentid');
							    		var param = {shipment_id:shipment_id};
							    		$.ajax({
							    			url:url,
							    			type: "POST",			 
							    			dataType: "JSON",
							    			...
							    			success: function(data){				
							    				if (data.success){	
							    					...
							    					$("#divDeleteForm").find('span').html('Đã xóa tổng cộng <b>'+data.totalorder + ' Order</b>.');
	...
	// ##############for paging######################
		...
		// show number of items per page
		 $(document).on('change','#rowPaginator',function(){
			 var url = $("#shipmentForm").attr('action');
			 // reset current page 
			 $('#page').val(1);
			 submitForm(url);
		 });
	...
	//########loads data##############
		 function submitForm(url){
			 var param = $("#shipmentForm").serialize();		
			 $.ajax({
					url:url,
					type: "POST",
					data: param,
